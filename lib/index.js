...
class Behavior {
    ...
    initLightDOM() {
        ...
        sliderText.innerHTML = `请依次点击  "${text === null || text === void 0 ? void 0 : text.split('').join('" "')}"`;
        // 增加loading
        const loadingContainer = createElement('div', 'loadingContainer');
        loadingContainer.style.width = `${width}px`;
        loadingContainer.style.height = `${height}px`;
    ...
    reset() {
        ...
        if ((type === 'puzzle' && (!this.canvas || !this.block)) ||
